...
gamma_param = 'scale'  # RBF kernel parameter
...
print(f"Starting RBF SVM evaluation (n={n}, kernel=rbf, gamma={gamma_param})")
...
for a in t_list:
    ...
    if a % turns == 0:
        ...
        svm_model = train_rbf_svm_pauli(
            n, phi_appro, num_samples=num_s, class_weights=class_w, gamma=gamma_param
        )
        
        # Test classification
        label, score = test_ob.test_EW_SVM() if hasattr(test_ob, 'svm_linear_model') else ('N/A', 0.0)
        
        # For RBF, manually classify the initial state
        ...
        if score < 0:
            ...
            print(f'Stopping RBF SVM for limit={limit}')
            break

        print(f'✓ Entanglement detected (score={score:.4f}, label={label})')

print(f"\nRBF SVM Evolution completed. Final results:")
...
if results_by_step:
    ...
    # Save CSV
    csv_path = os.path.join(results_dir, 'summary_rbf_svm_results.csv')
    df.to_csv(csv_path, index=False)
    print(f'Saved RBF results to {csv_path}')
    
    # Plot helper function
    def save_plot(x_col, y_col, xlabel, ylabel, title):
        plt.figure(figsize=(8, 5))
        plt.plot(df[x_col], df[y_col], marker='o', color='blue')
        plt.xlabel(xlabel)
        plt.ylabel(ylabel)
        plt.title(title + ' (RBF Kernel)')
        plt.grid(True)
        png_path = os.path.join(results_dir, f'{y_col}_vs_{x_col}_rbf.png')
...
print("RBF SVM analysis complete!")
