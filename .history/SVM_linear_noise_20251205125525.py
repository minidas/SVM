...
for step_idx, per_noise_dict in noise_results.items():
    for nt, step_data in per_noise_dict.items():
        ...
        # annotate DataFrame with round index for later aggregation
        ...
        try:
            # extract clean-state score (p_noise == 0) if present
            if 'p_noise' in df_noise.columns and 'score' in df_noise.columns and not df_noise.empty:
                try:
            ...
            summary_rows.append({'noise_type': nt, 'round': step_idx, 'max_noise': max_noise, 'clean_score': float('nan')})

        # Plot entropy vs p_noise (only if entropy was computed)
