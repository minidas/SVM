...
def main():
    ...
    # For each noise model, collect data across all runs and save combined CSV
    combined_data_by_noise = {}
    summary_rows = []

    # Single round (no time evolution loop like SVM_linear_noise.py)
    step_idx = 0
    for nt in noise_types:
        ...
        if nt not in combined_data_by_noise:
            combined_data_by_noise[nt] = []
        combined_data_by_noise[nt].append(df)

        # Score plot (only if --save-plots)
        if args.save_plots:
            ...
            plt.title(f'Commutator-SVM Score vs Noise ({nt})')
            plt.legend()
            plt.grid(True)
            plot_path = os.path.join(results_dir, f'comm_noise_{nt}_n{n}_score.png')
        ...
        if args.compute_entropy and args.save_plots and 'entropy' in df.columns:
            ...
            plt.xlabel('p_noise')
            plt.ylabel('Entropy')
            plt.title(f'Commutator-SVM Entropy vs Noise ({nt})')
            plt.legend()
            plt.grid(True)
            plot_path2 = os.path.join(results_dir, f'comm_noise_{nt}_n{n}_entropy.png')
    ...
    for nt in noise_types:
        if nt in combined_data_by_noise and len(combined_data_by_noise[nt]) > 0:
            ...
            combined_fname = f'comm_noise_combined_{nt}_n{n}_samples{num_s}.csv'
