...
def sample_random_pure_separable_state(n, seed=None, return_statevec=False):
    ...
    return rho_sep, rho_left, rho_right

def train_linear_svm_pauli(n, rho_approx, num_samples=1000, class_weights=(1.0, 1.0), seed=None, C=1.0, return_model=True):
    """
    ...
    for i in range(num_samples):
        rho_sep, _, _ = sample_random_pure_separable_state(n, seed=None)
    ...
    try:
        ...
        # Fallback to cvxpy formulation of weighted soft-margin linear SVM
        # Minimize 0.5||w||^2 + C * sum_i weight(y_i) * xi_i
        try:
            import cvxpy as _cp
        except Exception:
        ...
        constraints = [(_cp.multiply(y, X @ w + b_var) >= 1 - xi)]
        ...
        solution = {'w': w_val, 'b': b_val, 'status': prob.status}
        result = {
            'model': solution,
            'pauli_strings': pauli_strings,
            'X': X,
...
def classify_state_with_svm(state, trained_result, n=None, return_score=False):
    ...
    # Normalize and convert state to density matrix
    rho = None
    # qiskit Statevector
    try:
    ...
    # SparsePauliOp -> matrix
    try:
        from qiskit.quantum_info import SparsePauliOp as _SPO
    except Exception:
        _SPO = None

    if _SV is not None and isinstance(state, _SV):
        rho = _SV.to_operator(state).to_matrix()
    elif _SPO is not None and isinstance(state, _SPO):
        rho = state.to_matrix()
    else:
        # Allow passing the common tuple returned by
        # `sample_random_pure_separable_state` (rho_sep, rho_left, rho_right[, psi])
        if isinstance(state, (list, tuple)):
            candidate = None
            for s in state:
                if s is None:
                    continue
                # qiskit objects handled above; try numpy-like arrays
                try:
                    arr_s = np.asarray(s)
                except Exception:
                    continue
                if arr_s.ndim in (1, 2):
                    candidate = s
                    break
            if candidate is None:
                raise ValueError('Cannot find a suitable state array inside the provided sequence input.')
        ...
        if arr.ndim == 1:
            ...
            # assume density matrix
            rho = arr
        else:
            raise TypeError('`state` must be a statevector (1D) or density matrix (2D), or a qiskit Statevector/SparsePauliOp, or a sequence containing one of these.')
    ...
    if 2 ** n != dim:
        raise ValueError(f'Inferred dimension {dim} is not compatible with 2**n for n={n}.')

    # Build Pauli operator matrices in same order as training
    P_mats = [SparsePauliOp.from_list([(p, 1)], num_qubits=n).to_matrix() for p in pauli_strings]

    # Compute feature vector (expectation values)
    x = np.array([float(np.real(np.trace(P @ rho))) for P in P_mats], dtype=float)

    # Compute score and prediction
    pred_label = None
    score = None

    # sklearn SVC-like object: use decision_function / predict
    if hasattr(model, 'decision_function') and hasattr(model, 'predict'):
        ...
        try:
            score = float(model.decision_function(X_feat)[0])
        except Exception:
            # Some sklearn wrappers return a (n_samples, ) directly
            score = float(model.decision_function(X_feat))
        pred = int(model.predict(X_feat)[0])
        pred_label = 'entangled' if pred == 1 else 'separable'
    else:
        # Assume cvxpy fallback solution dict with 'w' and 'b'
