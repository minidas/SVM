...
def main():
    ...
    for nt in noise_types:
        ...
        # Plot entropy as scatter colored by detection label
        plt.figure(figsize=(8,5))
        ...
        # Collect summary info for this noise type
        summary_row = {
            'n': n,
            'num_samples': num_s,
            'limit': limit,
            ...
            'steps': steps,
            'noise_type': nt,
            'truncated_size': int(truncated.size) if hasattr(truncated, 'size') else -1,
    ...
    # Save aggregated summary CSV with per-noise results and basic run info
    try:
        summary_path = os.path.join(results_dir, f'commutator_noise_summary_n{n}_samples{num_s}.csv')
