...
def generate_noise_filename(n, limit, turns, train_step, noise_type='white'):
    cw_str = f"n{n}_limit{limit}_turns{turns}_step{train_step}_noise{noise_type}"
...
def main():
    ...
    parser.add_argument('--noise-type', type=str, default='white', choices=['white','bitflip','phaseflip','bitphase','amplitude'], help='Noise model to apply')
    parser.add_argument('--steps-save', type=int, default=10, help='Train step label used in filenames')
    ...
    results = {}
    noise_results = {}
    # Collect DataFrames across rounds to produce a single combined file for this noise_type
    combined_noise_list = []
    ...
    for a in t_list:
        ...
        if a % turns == 0:
            ...
            W = test_ob.Wppt_tog
            print(f"  Performing noise robustness test for WPPT at step {a} (noise={noise_type})...")
            noise_data, max_noise = find_noise_tolerance_for_witness(W, test_ob, n, noise_type=noise_type, compute_entropy=args.compute_entropy)
    ...
    for step_idx, step_data in noise_results.items():
        ...
        # Annotate round and add to combined list
        try:
            df['round'] = int(step_idx)
        except Exception:
            df['round'] = step_idx
        combined_noise_list.append(df)
        ...
        if args.save_plots:
            ...
            plt.legend()
            plt.grid(True)
            plot_path = os.path.join(results_dir, f'noise_wppt_{noise_type}_n{n}_step{step_idx}_trace.png')
        ...
        if args.compute_entropy and args.save_plots and 'entropy' in df.columns:
            ...
            plt.grid(True)
            plot_path2 = os.path.join(results_dir, f'noise_wppt_{noise_type}_n{n}_step{step_idx}_entropy.png')
            plt.savefig(plot_path2, bbox_inches='tight')
            print(f'  Saved plot to {plot_path2}')
            plt.close()

        # Append summary entry for this noise type and training round
        try:
            summary_rows.append({'noise_type': noise_type, 'round': int(step_idx), 'max_noise': float(max_noise)})
        except Exception:
            summary_rows.append({'noise_type': noise_type, 'round': step_idx, 'max_noise': max_noise})
    ...
    # Write combined CSV aggregating all rounds for this noise_type
    if len(combined_noise_list) > 0:
        try:
            combined_df = pd.concat(combined_noise_list, ignore_index=True)
        except Exception:
            combined_df = pd.DataFrame([r for sub in combined_noise_list for r in sub.to_dict(orient='records')])
        combined_fname = f'noise_wppt_combined_{noise_type}_n{n}_limit{limit}_turns{turns}_maxround{max_round}.csv'
