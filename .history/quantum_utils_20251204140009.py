...
class H_stab_state_evol:
    ...
    def SVM_linear(self, num_samples, class_weights):
        self.svm_linear_model = train_linear_svm_pauli(self.n, self.stabsum_tog, num_samples=num_samples, class_weights=class_weights)

    def test_EW_SVM(self, n_round =6):
        state_test = Statevector.to_operator(self.ini_state).to_matrix()
        result = classify_state_with_svm(state_test, self.svm_linear_model, return_score=True)
        print('tested result:', result)
        return result
